// Centralized gallery image list shared across pages/sections.
// Keep this file as the single source of truth for gallery ordering.

import img1 from "@/assets/science-festival.jpg";
import img2 from "@/assets/about-photo.jpg";
import img3 from "@/assets/project-electronics.jpg";
import img4 from "@/assets/science-fair-team.jpg";
import img5 from "@/assets/hydrover-prototype.jpg";
import img6 from "@/assets/hydrover-electronics.jpg";
import img7 from "@/assets/robot-car.jpg";
import img8 from "@/assets/award-ceremony.jpg";
import img9 from "@/assets/trophy-photo.jpg";
import img10 from "@/assets/childhood-photo.jpg";
import img12 from "@/assets/truemedi-prototype.jpg";
import img13 from "@/assets/bdrcs-volunteer.jpg";
import img14 from "@/assets/science-fair-presentation.jpg";
import img15 from "@/assets/robotics-workspace.jpg";
import img16 from "@/assets/startup-summit.jpg";
import img17 from "@/assets/family-childhood.jpg";
import img18 from "@/assets/siblings-swing.jpg";
import img19 from "@/assets/night-selfie.jpg";
import img20 from "@/assets/science-fair-team.jpg";
import img21 from "@/assets/electronics-experiment.jpg";
import img22 from "@/assets/smart-city-model.jpg";
import img23 from "@/assets/aquaguard-device.jpg";
import img25 from "@/assets/robot-car-selfie.jpg";
import img26 from "@/assets/club-activity.jpg";
import img30 from "@/assets/IMG_0329.jpg";
import img31 from "@/assets/IMG_2826.jpg";
import img32 from "@/assets/IMG_3143.JPG";
import img33 from "@/assets/IMG_3612 3.jpg";
import img34 from "@/assets/IMG_4720_Original.jpg";
import img35 from "@/assets/IMG_C1E7806B13C8-1.jpeg";
import img36 from "@/assets/IMG_EB5A4AB44D0A-1.jpeg";
import img37 from "@/assets/gallery-extra-1.png";
import img38 from "@/assets/gallery-extra-2.png";
import img39 from "@/assets/gallery-extra-3.png";
import img40 from "@/assets/gallery-extra-4.png";
import img41 from "@/assets/gallery-extra-5.png";
import img42 from "@/assets/gallery-extra-6.png";
import img43 from "@/assets/gallery-extra-7.png";
import img44 from "@/assets/gallery-extra-8.png";
import img45 from "@/assets/gallery-extra-9.png";
import img46 from "@/assets/gallery-extra-10.png";
import img47 from "@/assets/gallery-extra-11.png";
import img48 from "@/assets/gallery-extra-12.png";
import img49 from "@/assets/gallery-extra-13.png";
import img50 from "@/assets/gallery-extra-14.png";
import img51 from "@/assets/gallery-extra-15.png";
import img52 from "@/assets/gallery-extra-16.png";
import img53 from "@/assets/gallery-extra-17.png";
import img54 from "@/assets/gallery-extra-18.png";
import img55 from "@/assets/gallery-extra-19.png";
import img56 from "@/assets/gallery-extra-20.png";
import img57 from "@/assets/gallery-extra-21.png";
import img58 from "@/assets/gallery-extra-22.png";

export type GalleryImage = {
  src: string;
  alt: string;
};

export const galleryImages: GalleryImage[] = [
  { src: img1, alt: "Science Festival" },
  { src: img2, alt: "About Photo" },
  { src: img3, alt: "Electronics Development" },
  { src: img4, alt: "Team Collaboration" },
  { src: img5, alt: "HydroVer Prototype" },
  { src: img6, alt: "HydroVer Electronics" },
  { src: img7, alt: "Robot Car Project" },
  { src: img8, alt: "Award Ceremony" },
  { src: img9, alt: "Trophy Achievement" },
  { src: img10, alt: "Childhood Photo" },
  { src: img12, alt: "TrueMedi Prototype" },
  { src: img13, alt: "BDRCS Volunteer" },
  { src: img14, alt: "Science Fair Presentation" },
  { src: img15, alt: "Robotics Workspace" },
  { src: img16, alt: "Startup Summit" },
  { src: img17, alt: "Family Childhood" },
  { src: img18, alt: "Siblings on Swing" },
  { src: img19, alt: "Night Selfie" },
  { src: img20, alt: "Science Fair Team" },
  { src: img21, alt: "Electronics Experiment" },
  { src: img22, alt: "Smart City Model" },
  { src: img23, alt: "AquaGuard Device" },
  { src: img25, alt: "Robot Car Selfie" },
  { src: img26, alt: "Club Activity" },
  { src: img30, alt: "Photo 1" },
  { src: img31, alt: "Photo 2" },
  { src: img32, alt: "Photo 3" },
  { src: img33, alt: "Photo 4" },
  { src: img34, alt: "Photo 5" },
  { src: img35, alt: "Photo 6" },
  { src: img36, alt: "Photo 7" },
  { src: img37, alt: "Extra Photo 1" },
  { src: img38, alt: "Extra Photo 2" },
  { src: img39, alt: "Extra Photo 3" },
  { src: img40, alt: "Extra Photo 4" },
  { src: img41, alt: "Extra Photo 5" },
  { src: img42, alt: "Extra Photo 6" },
  { src: img43, alt: "Extra Photo 7" },
  { src: img44, alt: "Extra Photo 8" },
  { src: img45, alt: "Extra Photo 9" },
  { src: img46, alt: "Extra Photo 10" },
  { src: img47, alt: "Extra Photo 11" },
  { src: img48, alt: "Extra Photo 12" },
  { src: img49, alt: "Extra Photo 13" },
  { src: img50, alt: "Extra Photo 14" },
  { src: img52, alt: "Extra Photo 16" },
  { src: img53, alt: "Extra Photo 17" },
  { src: img54, alt: "Extra Photo 18" },
  { src: img55, alt: "Extra Photo 19" },
  { src: img56, alt: "Extra Photo 20" },
  { src: img57, alt: "Extra Photo 21" },
  { src: img58, alt: "Art Gallery Photo" },
];
